...
# Airbyte Docker
docker_host = "unix:///var/run/docker.sock"   # override DOCKER_HOST
image_prefix = "airbyte/"                     # private registry prefix
...
| `docker_host` | `$DOCKER_HOST` | Docker socket for local connector containers |
| `image_prefix` | `airbyte/` | Connector image prefix (for private registries) |
| `pull_policy` | `if-not-present` | When to pull connector images |
| `prefer_airbyte` | `true` | Route network sources through Airbyte when Docker or Cloud is configured |
