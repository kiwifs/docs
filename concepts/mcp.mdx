...
description: "Connect Claude, Cursor, and other AI tools to KiwiFS — stdio, HTTP, and 71 tools."
icon: "plug"
---
...
The server currently registers **71** tools (exact count may change release to release). They are grouped below; every name is invoked as listed.
...
| `kiwi_search_hybrid` | RRF fusion of FTS and semantic ranks. |
| `kiwi_search_semantic` | Vector similarity search (requires embeddings). |
| `kiwi_brief` | Token-budgeted context pack — never summarizes. |
