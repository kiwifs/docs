...
```markdown
:::claim{evidence=stated confidence=0.9 source=sources/architecture.md}
A non-linear level-2 stacker wins when the dominant feature is sparse.
:::

Inline: :claim[hill-climbing is worse]{evidence=inferred confidence=0.6}
```

| Attribute | Meaning |
| --- | --- |
| `evidence` | `stated`, `inferred`, or similar — changes badge tone |
| `confidence` | `0`–`1`, shown as a percentage |
| `source` | Path or citation; omitted claims are marked **unsourced** |
...
```markdown
:::figure{width=wide pin caption="Architecture overview"}
![[diagrams/arch.excalidraw.md|wide]]
:::

![[canvases/flow.canvas.json]]
```
...
parts:
  - chapter-01/_index.md
  - chapter-01/intro.md
  - chapter-02/_index.md
```
