...
| `X-Kiwi-Source` | Page view attribution on `GET /file` | `mcp` |
...
`/health` and `/healthz` return a simple OK payload:

```json
{"status": "ok"}
```

`/healthz` also includes uptime and build version:

```json
{"status": "ok", "uptime": "2h15m30s", "version": "0.20.0"}
```

`/readyz` verifies storage is reachable. When NFS, S3, or WebDAV is enabled, it also probes those protocol servers:
...
```json
{
  ...
  "protocols": {
    ...
    "s3": { "enabled": true, "healthy": true, "port": 3334 }
  }
}
```

Returns **`503`** when storage is unreachable or an enabled protocol fails its health check:

```json
{
  "status": "protocol-unhealthy",
  "protocols": {
    "nfs": { "enabled": true, "healthy": false, "port": 2049, "error": "connection refused" }
  }
}
```
