...
| `POST` | `/import/connections/:id/sync` | Pause, resume, or change auto-sync interval. Body: `{ "enabled": true, "interval": "1h" }`. |
| `GET` | `/import/sync/status` | List all sync-enabled connections with status (`idle`, `running`, `error`). |

### Auto-sync

Syncable sources (`postgres`, `mysql`, `mongodb`, `firestore`, `firebase-rtdb`, `notion`, `airtable`) enable periodic auto-sync on first import. The default interval is **1 hour**. Periodic syncs use full sync — pages from the same `_source` that are not seen in a run receive `_archived_at` frontmatter instead of being deleted.
...
See [Import connections](/import/connections) and [Airbyte import](/import/airbyte).
