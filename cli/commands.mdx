...
Export your knowledge base as structured data (JSONL, CSV, Parquet) or render documents (PDF, HTML, slides, static site).
...
**Document export flags** (require `--path`)
...
| `--crossref` | `false` | Enable cross-reference processing |
| `--pdf-engine` | auto | PDF engine: `typst` or `xelatex` |
| `--slide-format` | `html` | Slide output: `html`, `pdf`, or `pptx` |
| `--site-name` | | Static site title (`site` format) |
| `--site-url` | | Canonical site URL |
| `--repo-url` | | Repository URL for edit links |
...
kiwifs export --format parquet -o knowledge.parquet
```
...
```bash Render static site
kiwifs export --format site --path docs/ --output site.zip --site-name "Team wiki"
```
</CodeGroup>

See [Data export](/export/overview) and [Document export](/export/documents).
...
Display a dashboard summarizing knowledge health, link coverage, and engagement metrics.
...
|------|-------|---------|-------------|
| `--root` | `-r` | `./knowledge` | Knowledge root |
| `--scope` | | | Path prefix to scope results |
...
Text output includes **Engagement** metrics: total page views, top viewed pages, and failed searches (requires sqlite search backend).
