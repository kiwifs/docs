...
<Steps>
  ...
  <Step title="Initialize a knowledge base">
    ```bash
    ...
    <Tip>
    Templates: `kb`, `wiki`, `memory`, `tasks`, `data`, `cms`, `runbook`, `adr`, `prompt`, `research`, `log`, and `blank`. See [Init templates](/guides/templates).
    </Tip>
  </Step>
  ...
  <Step title="Write from your agent">
    <CodeGroup>
    ```bash Via REST
...
Your agent now has access to **71 MCP tools** for reading, writing, searching, querying, drafts, and more. See [MCP](/concepts/mcp) for the full tool reference.
